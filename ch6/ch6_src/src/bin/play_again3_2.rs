use libc::{
    fcntl, sighandler_t, signal, tcgetattr, tcsetattr, termios, ECHO, F_GETFL, F_SETFL, ICANON, O_NDELAY, SIGINT, SIGQUIT, SIGTSTP, TCSANOW, VMIN
};
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    unsafe {
        ...
        signal(SIGTSTP, handler as sighandler_t);
        signal(SIGQUIT, handler as sighandler_t);
    }
    tty_mode(0)?;
    set_cr_noecho_mode()?;
    set_nodelay_mode()?;
